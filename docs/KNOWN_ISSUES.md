...
- [ ] Generator functions (`function*`/`yield`) are still unsupported — they throw a visible `Unsupported syntax` error rather than executing incorrectly. This is the last major gap; supporting it well would require rethinking the generator-based execution model further, since the interpreter already uses JS generators internally to drive its own suspend/resume for `async`/`await`, and a user-level `function*` would need to be visualized as a *separate* suspendable execution unit (with its own place in the visualizer's queues) rather than reusing that same internal mechanism directly.
